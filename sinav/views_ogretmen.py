...
def ogretmen_sinav_gozetim(request):
    ...
    if not ogretmen_adi:
        return render(request, "sinav/ogretmen_sinav_gozetim.html", {
...
def ogretmen_sinav_medya(request):
        ...
        if _sim_tarih and _sim_saat:
            try:
                simdi_override = timezone.make_aware(
                    datetime.strptime(f"{_sim_tarih} {_sim_saat}", "%Y-%m-%d %H:%M")
                )
                sim_aktif = True
            except ValueError:
                pass
        ...
        for slot in slots:
            ...
            for sev in sorted(seviyeler):
                ...
                if m.serbest:
                    ...
                    sinav_saat = datetime.strptime(m.takvim.saat, "%H:%M").time()
                    sinav_dt   = timezone.make_aware(
                        datetime.combine(m.takvim.tarih, sinav_saat)
                    )
                    erisim_acik = (
                        sinav_dt
                        <= simdi <=
                        sinav_dt + timedelta(minutes=TOLERANS_DAKIKA)
                    )
                medyalar.append({
                    "obj":          m,
    ...
    return render(request, "sinav/sinav_medya.html", {
        "title":          "Sınav Medyası",
        "ogretmen_adi":   ogretmen_adi,
        "aktif_sinav":    aktif_sinav,
...
def ogretmen_gozetim_sinif_listesi(request):
    ...
    return render(request, "sinav/ogretmen_gozetim_sinif_listesi.html", {
...
def sinav_salon_yoklama(request):
    ...
    if ogrenciler:
        ...
        seats_per_group = -(-max_sira // N_GROUPS)
        seats_per_group = -(-seats_per_group // N_COLS) * N_COLS
    ...
    return render(request, "sinav/sinav_salon_yoklama.html", {
...
def sinav_oturum_istatistik(request):
    ...
    return render(request, "sinav/sinav_oturum_istatistik.html", {
...
def ogretmen_yoklama_raporum(request):
    ...
    personel = getattr(request.user, "personel", None)
    if not personel:
        return render(request, "sinav/ogretmen_yoklama_raporum.html", {
...
def sinif_oturma_plani(request):
    ...
    if user.is_superuser and sinif_sube_id:
        ...
        return render(request, "sinav/sinif_oturma_plani.html", {
    ...
    if request.method == "POST":
        ...
        _messages.success(request, f"{sinif_sube} sınıfının oturma düzeni kaydedildi.")
...
def ogretmen_ogrenci_sinav_takvimi(request):
    ...
    personel = getattr(request.user, "personel", None)
    if not personel:
        return render(request, "sinav/ogretmen_ogrenci_sinav_takvimi.html", {
