...
Aktif TakvimUretim'deki tüm (ders, sinav_turu) çiftlerini,
tanımlanmış MazeretOturum'lara dengeli şekilde dağıtır.

Kurallar:
- Yazılı dersler  → sinav_turu='Yazili' olan oturumlara
- Uygulama dersleri → sinav_turu='Uygulama' olan oturumlara
- Uygulama oturumları aynı gün başka oturumla çakışamaz
  (MazeretOturum oluşturulurken enforce edilir — bkz. views.py)
...
def dagit(mazeret_sinav: MazeretSinav) -> Tuple[bool, str]:
    """
    Aktif TakvimUretim'deki sınavları MazeretOturum'lara round-robin ile dağıtır.
    ...
    # Unique (ders_id, sinav_turu) çiftleri
    takvim_kayitlar = list(
        Takvim.objects.filter(uretim=aktif_uretim)
        .values("ders_id", "sinav_turu")
        .distinct()
        .order_by("sinav_turu", "ders_id")
    )
    ...
    return True, (
        f"{len(yazili_dersler)} Yazılı ve {len(uygulama_dersler)} Uygulama dersi "
